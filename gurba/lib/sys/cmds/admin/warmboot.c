...
void next_stage(int count, object player) {
   ...
   switch (count) {
      case 0:
         ...
         if (!error) {
            player->message("Validate Upgrade Successful");
            error = catch(rebuild_critical(player));
         } else {
            player->message("Validate Upgrade Failed");
            player->message(error);
         }
         break;
      case 1:
         player->message("Rebuild Critical Successful");
         error = post_kernel_upgrade(player);
         if (error) {
            player->message("Rebuild Critical Failed");
            player->message(error);
         }
         break;
      case 2:
         player->message("Post Kernel Upgrade Successful");

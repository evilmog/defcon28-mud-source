...
string build_string_mxp(string str, object obj, string num, string *objs) {
   int val;

   val = obj->query_value();
   if (!obj->query_Name()) {
      if (!obj->query_adj() || obj->query_adj() == "") {
         str += " %^RED%^[" + num + "]%^RESET%^ <Vendor>" + obj->query_id() +
            "</Vendor>, " + val + " credit" + credits(val) + ".\n";
      } else {
         str += " %^RED%^[" + num + "]%^RESET%^ " +
            obj->query_adj() + " <Vendor>" + obj->query_id() + "</Vendor>, " +
            val + " credit" + credits(val) + ".\n";
      }
   } else {
      val = objs->query_value();
      str += " %^RED%^[" + num + "]%^RESET%^ <Vendor>" + obj->query_Name() +
         "</Vendor>, " + val + " credit" + credits(val) + ".\n";
   }
   ...
   for (i = 0; i < maxi; i++) {
      ...
      if (stored_items[objs[i]] > 0) {
         ...
         if (obj) {
            ...
            str = build_string_mxp(str, obj, num, objs);
...
void do_list(object player) {
   ...
   if (mxp) {
     out("%^MXP_LSM%^<!EL Vendor \'<send \"buy &text;\" prompt>\'>");
   }
   write("%^BLUE%^=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=" +
      "-=-=-=-=-=-=-=-=-=-=-=-%^RESET%^\n\n");
   if(mxp) {
   	write(query_list_mxp());
   } else {
	write(query_list());
   }
   if (mxp) {

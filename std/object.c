...
string query_long(void) {
   if (this_player()->query_mxp() == 1 && long_desc_mxp) {
     return long_desc_mxp;
   }
   return long_desc;
}

void set_long_mxp(string str) {
   long_desc_mxp = str;
}

string query_long_mxp(void) {
   if (long_desc_mxp) {
     return long_desc_mxp;
   } else {
     return long_desc;

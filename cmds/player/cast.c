...
string *usage(string str) {
   ...
   lines += get_alsos();
   /* override as we are disabling magic */
   lines = ({ "You can't cast magic, there is no such thing." });
   return lines;
}
...
static int has_spell(string spellname) {
   ...
   /* Disable all magic, this is colony 1.4 */
   return 0;
...
static void list_spells(void) {
   ...
   /* neuter spells */
   write("There's no such thing as magic");
   return;
...
static void cast_spell(string spell, string who) {
   ...
   /* Remove Magic From Game - colony 1.4 */
   return;
